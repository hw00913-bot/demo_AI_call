...
(function () {
  ...
  /* ===== 任务详情渲染（按 中科金 API 字段） ===== */
  const RecallStatusLabels = { '5': '未接听', '9': '占线', '12': '无法接通', '14': '停机', '15': '关机', '6': '拨打失败', '11': '来电提醒' };
  ...
  function renderTaskDetail() {
    ...
    if (item.platform === '中科金智能') {
      ...
      if (d.blacklistGroupIds) {
        html += '\n          <div class="task-detail-row"><div class="task-detail-label">黑名单组</div><div class="task-detail-value">' + d.blacklistGroupIds + '</div></div>';
      }
      if (d.interceptStrategyCode) {
        var strategyName = d.interceptStrategyCode;
        var strategies = window.MockZkjInterceptStrategies || [];
        var found = strategies.find(function (s) { return s.strategyCode === d.interceptStrategyCode; });
        if (found) strategyName = found.name + '（' + d.interceptStrategyCode + '）';
        html += '\n          <div class="task-detail-row"><div class="task-detail-label">拦截策略</div><div class="task-detail-value">' + strategyName + '</div></div>';
      }
