...
2. **中科金平台的字段**，参照冰兰版中 **一知科技** 的面板结构（`platformPanelDefault`）：
   ...
   - 提示 notice
   - 模型类型（Radio: 小模型/大模型）
   - 选择中科金账号（Select，disabled，默认值「默认账号」，选中模型类型后显示）
   - 即中科金字段 = 一知科技的字段结构，不参照冰兰的复杂字段（呼叫通道/线路/时段/重拨等）。

3. **平台面板切换逻辑**（`onPlatformChange`）：四个平台（冰兰/科大讯飞/一知科技/中科金智能）各自控制对应面板的显隐，互斥展示。切换平台时清空场景传入信息表格；选中中科金智能时自动填充默认字段（姓名 / name / 必填）。

4. **外呼列表** 和 **通话记录** 的页面结构，参照冰兰版对应文件。

5. **已呼叫 → 通话记录弹窗**：两层抽屉结构，第一层为该号码的通话记录列表（右侧滑入抽屉，80%宽度），第二层为通话详情（右侧滑入抽屉，左音频+对话，右外呼结果/详细信息Tab）。

### 必填标识规范

- 表单标签必填：`biz-form-label.required` 伪元素红色星号 `*`
- 表格内必填：`<span class="biz-required-tag">* 是</span>` 红色显示
- Tab 名称必填：`<span class="biz-required-tag">*</span>` 前缀
- 不可删除字段：`<span class="biz-action-disabled">删除</span>` 置灰
