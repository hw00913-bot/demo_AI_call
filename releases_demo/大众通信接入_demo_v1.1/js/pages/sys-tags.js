...
window.Pages['sys-tags'] = (function() {
  ...
  function buildTagListHTML(supplierId, tenantType, sceneId) {
    ...
    var isDazhong = supplierId === 'dazhong';
    ...
    if (isDazhong) html += '<span data-anno="sys-tags-dazhong-manual" style="padding:6px 12px;background:#e6f4ff;color:#1677ff;border:1px solid #91caff;border-radius:4px;font-size:12px;">中台手工配置</span>';
    ...
    html += isDazhong
      ? '大众通信标签由管理员在中台手工新增、编辑、删除、排序和启停，不从接口自动读取；标签到本地标准标签的关系也必须手动映射。'
      : '当前在供应商标签集下配置 <b>' + supplier.name + '</b> 的标签启用状态和映射关系。';
    html += '</div>';
  ...
  function ensureMappingsForEnabledTags(supplierId, tenantType, sceneId) {
    if (supplierId === 'dazhong') return; // 大众通信禁止自动映射
