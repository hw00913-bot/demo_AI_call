...
(function () {
  ...
  const MockSceneRows = [
    ...
    { id: 11, name: '华东店-冷线索跟进-大众通信', sceneId: '9f6d9a40-2fb3-4c56-8b21-202607140017', code: 'DZ-SH-COLD', category: '冷线索', tenant: '东风日产-燃油车', platform: '大众通信', task_type: 7, line: 'DT-SH-001', maximumcall: 20, billingType: '按通话时长', updater: '超管', updateTime: '2026-07-14 16:20:00' }
  ];
  ...
  let SceneRows = dazhongFirst(MockSceneRows).map(function (row) { return Object.assign({}, row); });

  function renderSceneRowsHtml() {
    return dazhongFirst(SceneRows).map(function (row, index) {
  ...
  /* ===== 新建/编辑业务场景抽屉 ===== */
  function upgradeDazhongPanel() {
    var panel = document.getElementById('platformPanelDazhong');
    if (!panel) return;
    panel.innerHTML = [
      '<div class="biz-form-row"><label class="biz-form-label required">大众通信任务ID</label><div class="biz-form-field"><input type="text" class="biz-form-input" id="dazhongTaskId" placeholder="请输入任务 uuid" data-anno="sys-scene-dazhong-taskid-runtime"></div></div>',
      ...
      '<div class="biz-form-row" id="dzModelTypeRow"><label class="biz-form-label required">模型类型</label><div class="biz-form-field"><div class="biz-radio-group"><label class="biz-radio"><input type="radio" name="dzModelType" value="小模型" onchange="window.Pages[\'sys-scene\'].onDzModelTypeChange()"><span>小模型</span></label><label class="biz-radio"><input type="radio" name="dzModelType" value="大模型" onchange="window.Pages[\'sys-scene\'].onDzModelTypeChange()"><span>大模型</span></label></div></div></div>',
  ...
  function submitAddModal() {
    ...
    var categoryRadio = document.querySelector('input[name="sceneType"]:checked');
    var dzIdInput = document.getElementById('dazhongTaskId');
    var zkjIdInput = document.querySelector('#platformPanelZhongkejin input[placeholder="请输入中科金智能平台创建的外呼任务id"]');
    var yzIdInput = document.querySelector('#platformPanelYizhi input[placeholder="请输入一知科技平台创建的自助场景id"]');
    
    var sceneId = 'DZ' + Date.now();
    if (platformRadio && platformRadio.value === '大众通信' && dzIdInput && dzIdInput.value) {
      sceneId = dzIdInput.value.trim();
    } else if (platformRadio && platformRadio.value === '中科金智能' && zkjIdInput && zkjIdInput.value) {
      sceneId = zkjIdInput.value.trim();
    } else if (platformRadio && platformRadio.value === '一知科技' && yzIdInput && yzIdInput.value) {
      sceneId = yzIdInput.value.trim();
    }

    if (platformRadio && platformRadio.value === '大众通信' && (!dzIdInput || !dzIdInput.value.trim())) {
      showToast('请输入大众通信任务 uuid', 'warning');
      return;
    }
    if (nameInput && nameInput.value.trim()) {
      var nextId = SceneRows.reduce(function(max, item) { return item.id > max ? item.id : max; }, 0) + 1;
      var newScene = {
        id: nextId,
        name: nameInput.value.trim(),
        sceneId: sceneId,
        code: codeInput ? codeInput.value.trim() : 'DZ-NEW',
        category: (categoryRadio ? categoryRadio.value : '冷线索'),
        tenant: '东风日产-燃油车',
        platform: (platformRadio ? platformRadio.value : '大众通信'),
        updater: '超管',
        updateTime: new Date().toISOString().replace('T', ' ').slice(0, 19),
        status: 'running'
      };
      SceneRows.push(newScene);
      if (window.MockSceneList) {
        window.MockSceneList.push({
          id: nextId,
          name: nameInput.value.trim(),
          status: 'running',
          source: '手动导入',
  ...
  function init() {
    var platformSelect = document.querySelector('.scene-list-page .filter-item:nth-child(3) select');
    if (platformSelect && !platformSelect.querySelector('option[value="大众通信"]')) {
      platformSelect.insertAdjacentHTML('beforeend', '<option value="大众通信">大众通信</option>');
    }
  }
  ...
  function onPlatformChange() {
    ...
    var panelDz = document.getElementById('platformPanelDazhong');
    ...
    var dzModelTypeRow = document.getElementById('dzModelTypeRow');
    var zkjAccountRow = document.getElementById('zkjAccountRow');
    var dzAccountRow = document.getElementById('dzAccountRow');

    // 全部隐藏
    if (panelDz) panelDz.classList.add('hidden');
    ...
    if (dzModelTypeRow) dzModelTypeRow.classList.add('hidden');
    if (zkjAccountRow) zkjAccountRow.classList.add('hidden');
    if (dzAccountRow) dzAccountRow.classList.add('hidden');
    ...
    // 清空大众模型类型
    var dzChecked = document.querySelector('input[name="dzModelType"]:checked');
    if (dzChecked) dzChecked.checked = false;
    ...
    } else if (platform.value === '大众通信') {
      if (panelDz) panelDz.classList.remove('hidden');
      if (dzModelTypeRow) dzModelTypeRow.classList.remove('hidden');
    }
  }
