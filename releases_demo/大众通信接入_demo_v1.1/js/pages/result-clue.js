...
(function () {
  ...
  function normalizeClueRow(item) {
    return Object.assign({}, item, {
      lastVisitTime: item.lastVisitTime || item.callTime || item.time || '-',
      visitCount: item.visitCount || 1,
      callStatus: item.callStatus || item.status || '-',
      record: item.record || item.summary || '-',
      sceneName: item.sceneName || item.scene || '-',
      firstVisitTime: item.firstVisitTime || item.callTime || item.time || '-',
      secondVisitTime: item.secondVisitTime || '-',
      thirdVisitTime: item.thirdVisitTime || '-'
    });
  }
  ...
  var allRows = dazhongFirst(staticRows.concat((window.MockClueDetailNEV || []).map(normalizeClueRow)));
  var rows = allRows.slice();

  function formatDazhongIntention(level) {
    var mapping = {
      1: 'A级',
      2: 'B级',
      3: 'C级',
    ...
    return mapping[level] !== undefined ? mapping[level] : '未评级';
  }
  ...
  function formatDazhongCallStatus(code) {
    ...
    return mapping[code] !== undefined ? mapping[code] : (code !== undefined && code !== null ? code : '-');
  }
  ...
  function getComponentText(component) {
    return ['value', 'text', 'content', 'result', 'answer', 'fieldValue'].map(function (key) {
      return cleanClueText(component[key]);
  ...
  function getDazhongDetail(item) {
    if (!item || item.platform !== '大众通信') return null;
    var callid = item.callid || item.sessionId || '';
    return callid && window.MockDazhongCallDetailByRecordId ? window.MockDazhongCallDetailByRecordId[callid] || null : null;
  }
  ...
  function renderClueTagPills(names) {
    if (!names || !names.length) return '<div class="clue-tag-field">-</div>';
    return '<div class="clue-tag-list clue-tag-list-component">' + names.map(function (name) {
  ...
  function getDazhongSceneCode(item) {
    var mapping = window.DazhongSceneCodeByScene || {};
    return item.sceneCode || mapping[item.sceneName] || mapping[item.scene] || '-';
  }

  function resolveDazhongCallContext(item) {
    var detail = getDazhongDetail(item) || {};
    var callid = item.callid || item.sessionId || detail.recordid || '-';
    var componentSummary = extractDazhongComponentSummary(detail);
    var componentTagNames = extractDazhongComponentTagNames(detail);
    var bailianSummary = cleanClueText(item.bailianSummary || detail.bailianSummary || item.summary || item.record);
    var bailianTagName = cleanClueText(item.bailianTagName || detail.bailianTagName || item.aiTagName);
    var dazhongTagName = cleanClueText(item.dazhongTagName || detail.dazhongTagName || item.aiTagName);
    return {
  ...
  function getClueVisitSummary(item) {
    if (item.platform === '大众通信') {
      return resolveDazhongCallContext(item).item.summary || '-';
    }
    return item.summary || item.record || '-';
  }
  ...
  function extractDazhongDialogRows(records) {
    ...
    return list.filter(function (record, index) {
      ...
      var question = cleanClueText(record.question);
      var answer = cleanClueText(record.answer_content);
      if (question) dialogRows.push({ role: '客户', text: question });
      if (answer) dialogRows.push({ role: record.bridge_status ? '人工客服' : 'AI客服', text: answer });
      return dialogRows;
    }, []);
  }
  ...
  function renderRows() {
    if (!rows.length) {
      return '<tr><td colspan="14"><div class="clue-empty">暂无线索记录</div></td></tr>';
    }
    return rows.map(function (item, index) {
      var displayLevel = item.level;
      if (item.platform === '大众通信') {
        displayLevel = formatDazhongIntention(item.intention_results);
      }
      var visitSummary = getClueVisitSummary(item);
      return '<tr>' +
        '<td>' + (index + 1) + '</td>' +
        ...
        '<td class="clue-record-cell" title="' + escapeHTML(visitSummary) + '">' + shortText(visitSummary) + '</td>' +
        '<td>' + escapeHTML(displayLevel) + '</td>' +
        ...
        '<td>' + escapeHTML(item.platform || '一知科技') + '</td>' +
  ...
  function showTags(index) {
    var item = rows[index] || {};
    if (item.platform === '大众通信') {
      showTagModal(renderClueTagPills(extractDazhongComponentTagNames(getDazhongDetail(item))));
      return;
    }
    ...
    var fields = [
      ['意向标签', detailTags.intent || displayLevel || '-'],
  ...
  function showDetail(index) {
    ...
    var visitDuration = item.platform === '大众通信' ? (item.duration || '-') : '16秒';
    var visitStartTime = item.platform === '大众通信' ? (item.startTime || item.firstVisitTime || '-') : ((item.firstVisitTime || '').replace(':10', ':17') || '-');
    var visitEndTime = item.platform === '大众通信' ? (item.endTime || '-') : ((item.firstVisitTime || '').replace(':10', ':56') || '-');
    var visitSummary = getClueVisitSummary(item);
    ...
    document.body.insertAdjacentHTML('beforeend', '<div class="clue-visit-backdrop" id="clueVisitBackdrop" onclick="window.Pages[\'result-clue\'].closeVisitModal(event)">' +
      ...
      '<td class="clue-visit-record" title="' + escapeHTML(visitSummary) + '">' + shortText(visitSummary) + '</td>' +
      '<td><a href="#" onclick="event.preventDefault();window.Pages[\'result-clue\'].showTagsByPhone(\'' + escapeHTML(item.phone || '') + '\')">查看</a></td>' +
      '<td>' + escapeHTML(displayLevel || '-') + '</td><td>' + escapeHTML(item.callStatus || '-') + '</td><td>' + escapeHTML(visitDuration) + '</td>' +
      '<td>' + breakText(visitStartTime, ' ') + '</td><td>' + breakText(visitEndTime, ' ') + '</td>' +
  ...
  function renderDialogRows(dialogRows) {
    ...
    return dialogRows.map(function (row) {
      return '<div class="clue-call-talk-row"><div class="clue-call-speaker">' + escapeHTML(row.role) + '</div><div class="clue-call-bubble">' + escapeHTML(row.text) + '</div></div>';
    }).join('');
  }

  function renderCallTranscript(item) {
    if (item.platform === '大众通信') {
      return renderDialogRows(extractDazhongDialogRows((getDazhongDetail(item) || {}).records));
    }
    var talks = [
      ['客服', '您好~'],
      ['客户', '东风日产来电'],
  ...
  function renderCallResult(item) {
    if (item.platform === '大众通信') {
      var context = resolveDazhongCallContext(item);
      var dazhongItem = context.item;
      return '<div class="clue-call-result">' +
        '<div class="clue-call-section-title">外呼小结</div>' +
        '<div class="clue-call-summary">' + escapeHTML(dazhongItem.summary || '-') + '</div>' +
        '<div class="clue-call-info-row"><span>意向标签：</span><b>' + escapeHTML(dazhongItem.aiTagName || '-') + '</b></div>' +
        '<div class="clue-call-info-row"><span>计划到店时间：</span><b>-</b></div>' +
        '<div class="clue-call-info-row"><span>预计购车时间：</span><b>-</b></div>' +
    ...
    var levelLabel = item.platform === '大众通信' ? '意向级别：' : '意向等级编码(A-E)：';
    var levelVal = item.platform === '大众通信' ? displayLevel : 'E';
    return '<div class="clue-call-result">' +
      '<div class="clue-call-section-title">外呼小结</div>' +
      ...
      '<div class="clue-call-info-row"><span>意向品牌中文名：</span><b>-</b></div>' +
      '<div class="clue-call-info-row"><span>意向车系中文名：</span><b>-</b></div>' +
      '<div class="clue-call-info-row"><span>' + levelLabel + '</span><b>' + escapeHTML(levelVal) + '</b></div>' +
      '</div>';
  }
  ...
  function renderCallInfo(item) {
    if (item.platform === '大众通信') {
      ...
      var fields = [
        ...
        ['用户号码', dazhongItem.phone || '-'],
        ['场景编码', getDazhongSceneCode(dazhongItem)],
        ['场景名称', dazhongItem.sceneName || dazhongItem.scene || '-'],
        ['对话时长', dazhongItem.duration || '-'],
        ['通话开始时间', dazhongItem.startTime || dazhongItem.callTime || '-'],
        ['通话结束时间', dazhongItem.endTime || '-'],
        ['通话结果', formatDazhongCallStatus(dazhongItem.call_status !== undefined ? dazhongItem.call_status : dazhongItem.status)],
        ['转人工状态', dazhongItem.transferStatus || '无转人工'],
        ['转人工时间', dazhongItem.transferTime || '-'],
        ['用户关注', dazhongItem.userConcern || '-'],
        ['意向标签', dazhongItem.dazhongTagName || '-'],
        ['通话标签', context.componentTagNames || []]
      ];
      return '<div class="clue-call-info-list">' + fields.map(function (field) {
        return '<div class="clue-call-info-row"><span>' + escapeHTML(field[0]) + '：</span><b>' + renderInfoValue(field[1]) + '</b></div>';
      }).join('') + '</div>';
    }
    var fields = [
  ...
  function showCallDetailByPhone(phone) {
    ...
    var isDazhong = item.platform === '大众通信';
    var context = isDazhong ? resolveDazhongCallContext(item) : null;
    var titleId = context ? context.callid : '2068963937235734529';
    var recordingDuration = context && context.item.recordingDuration ? context.item.recordingDuration : '0:15';
    ...
    document.body.insertAdjacentHTML('beforeend', '<div class="clue-call-backdrop" id="clueCallBackdrop" onclick="window.Pages[\'result-clue\'].closeCallDetail(event)">' +
      ...
      '<div class="clue-call-header"><button onclick="window.Pages[\'result-clue\'].closeCallDetail()">×</button><span>会话 id： ' + escapeHTML(titleId) + '</span></div>' +
      '<div class="clue-call-body">' +
      '<div class="clue-call-left">' +
      '<div class="clue-call-audio"><span>▶</span><span>0:00 / ' + escapeHTML(recordingDuration) + '</span><i></i><span>🔊</span><span>⋮</span><span class="clue-call-audio-icon">🎧</span><span class="clue-call-audio-icon">▣</span></div>' +
      '<div class="clue-call-transcript">' + renderCallTranscript(item) + '</div>' +
      '</div>' +
  ...
  function render() {
    return '<div class="result-clue-page">' +
      ...
      '<th>序号</th><th>用户号码</th><th>最后回访时间</th><th>回访次数</th><th>最后通话状态</th><th>最后回访记录</th><th>最后通话意向级别</th><th>客户详细标签</th><th>场景名称</th><th>智能平台</th><th>首次实际回访时间</th><th>二次实际回访时间</th><th>第三次实际回访时间</th><th class="clue-action-cell">操作</th>' +
  ...
  function init() {
    allRows = dazhongFirst(staticRows.concat((window.MockClueDetailNEV || []).map(normalizeClueRow)));
    rows = allRows.slice();
    updateLevelFilterVisibility();
  }
