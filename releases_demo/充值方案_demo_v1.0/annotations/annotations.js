window.AnnotationData = {
  "index": [
    {
      ...
      sections: {
        functionName: "用量余额概览",
        functionDesc: "首页固定展示门店「重庆东风南方渝兴」的用量概览，包含四张核心指标卡片：大模型可用分钟数、小模型可用分钟数、有效期和呼叫控制状态，帮助门店快速了解当前账户的可用通话资源。",
        permissionScope: "门店租户可查看自身用量；产品超管可通过左侧导航「首页」进入查看任一门店的演示数据。",
        dataSource: "租户价格规则、已生效充值单记录、冻结中的外呼任务、余额调整流水和呼叫控制配置。",
        valueLogic: "资金余额 = 已生效通话充值总额 - 手工扣减金额 - 已消费金额；可用金额 = max(资金余额 - 冻结总额, 0)；大模型/小模型可用分钟数分别 = 可用金额 ÷ 对应模型默认单价；有效期取已生效记录中最晚的截止日期；呼叫控制状态需同时满足有效期内、可用金额大于0且未被手动停用。",
        fieldDesc: "四张卡片分别展示大模型可用分钟数（蓝色大号数字+分钟单位）、小模型可用分钟数、有效期（YYYY-MM-DD ~ YYYY-MM-DD 格式）和呼叫控制状态（绿色「可发起」或红色「不可发起」标签）。",
        interactionDesc: "页面为只读展示，卡片无点击交互。底部引导区提供链接可跳转至系统管理 → 租户管理 → 充值管理查看完整计费信息。",
        judgeRule: "可用分钟数按两位小数格式化展示；有效期无已生效记录时显示「未生成」；呼叫控制状态由有效期、可用金额和手动开关三者共同决定。",
        exceptionRule: "单价未配置时分钟数显示 0.00；无已生效有效期时显示「未生成」；冻结覆盖不足时可用分钟数归零。",
        otherDesc: "页面内独立实现简化版计费计算，与租户管理页同源但独立维护。每次渲染前自动执行冻结任务释放检测。"
      }
    },
    {
      id: "2",
      page: "index",
      target: "[data-anno='home-billing-link']",
      position: { placement: "top-right", offsetX: 0, offsetY: 0 },
      title: "充值管理跳转",
      sections: {
        functionName: "充值管理跳转",
        functionDesc: "首页底部引导区提供快速跳转入口，点击链接可直接导航至系统管理的租户管理页面，查看完整的计费明细、充值单管理和余额调整功能。",
        permissionScope: "产品超管可操作；其他角色权限待确认。",
        dataSource: "前端路由导航。",
        valueLogic: "调用页面路由函数跳转至 sys-tenant 页面，面包屑同步更新为 系统管理 / 租户管理。",
        fieldDesc: "链接文案为「系统管理 → 租户管理 → 充值管理」，蓝色可点击样式。",
        interactionDesc: "点击链接后页面内容区切换为租户管理列表页，左侧导航高亮「系统管理 > 租户管理」。",
        judgeRule: "无特殊判断条件，任何角色均可点击跳转。",
        exceptionRule: "无。",
        otherDesc: "该链接为原型内页面间导航，不涉及外部系统跳转。"
      }
    }
  ],
  "interaction": [],
  "sys-tenant": [
    ...
    },
    {
      ...
      sections: {
        functionName: "手动生效",
        functionDesc: "对已关联的充值单逐条生效，按计费类型先确认有效时长、添加余额或二者组合，再更新有效期、余额或两者。",
        permissionScope: "产品超管可操作；其他角色权限待确认。",
        dataSource: "历史关联充值单、充值金额、套餐信息和租户已有有效期记录。",
        valueLogic: "仅坐席费更新有效期，仅通话费更新余额，组合单同时更新。含坐席费时半年套餐默认添加183日、全年套餐默认添加365日；含通话费时按充值金额反显添加余额，均可调整后再确认。",
        fieldDesc: "历史记录逐行展示充值单号、支付状态、生效状态、计费类型、套餐、周期、充值金额、有效期和操作；确认弹窗按类型展示有效期预览、添加有效时长、当前资金余额、冻结总和、添加通话余额和预计可用余额。",
        interactionDesc: "待生效记录显示“生效”按钮；仅坐席费弹出时长确认；仅通话费弹出余额确认；坐席费+通话费弹出联合确认。完成后刷新租户列表、计费明细和历史记录。",
        judgeRule: "充值单必须已支付、已经关联且当前尚未生效；添加有效时长必须为大于0的整数，添加通话余额必须大于0。",
        exceptionRule: "充值单未支付、记录不存在、已经生效、有效时长不合法或添加余额不合法时不允许完成生效。",
    ...
    },
    {
      id: "7",
      page: "sys-tenant",
      target: "[data-anno='tenant-billing-export']",
      position: { placement: "bottom-left", offsetX: 0, offsetY: 0 },
      title: "计费汇总导出",
      sections: {
        functionName: "计费汇总导出",
        functionDesc: "导出全部租户的计费与呼叫状态汇总，便于线下核对和报表说明。",
        permissionScope: "产品超管可操作；其他角色权限待确认。",
        dataSource: "租户基础信息及页面实时计费汇总结果。",
        valueLogic: "扣费总额=实际消费金额+已生效手工扣减金额；冻结总和为有效冻结金额；可用余额=max（历史充值总额-扣费总额-冻结总和，0）；大小模型可用分钟数分别按对应默认单价换算。",
        fieldDesc: "导出租户名称、租户ID、租户类型、历史充值总额、扣费总额、冻结总和、可用余额、大小模型可用分钟数、有效期和呼叫控制状态。",
        interactionDesc: "点击租户列表右上角“导出”按钮，下载带UTF-8 BOM的CSV文件。",
        judgeRule: "导出数据必须复用当前页面计费汇总函数，金额和分钟数保留两位小数。",
        exceptionRule: "模型默认价未配置时，对应可用分钟数输出“未配置”。",
        otherDesc: "文件名包含导出日期，可直接使用Excel打开。"
      }
    }
  ]
};
