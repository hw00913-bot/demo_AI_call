  ...
  "sys-tags": [
    {
      ...
      sections: {
        ...
        otherDesc: "目前中台标签以百炼标签为准进行初始化，枚举如下：A (高意向)B (潜在)C (一般)D (忙碌/敷衍)E (拒绝/无效/无应答)。"
      },
  ...
  "result-clue": [
    {
      ...
      title: "场景名称与意向级别筛选",
      sections: {
        functionName: "场景名称与多选级别筛选",
        functionDesc: "用于在线索记录中选择具体外呼场景，并驱动后续「最后通话用户意向级别（多选）」筛选项的显示和取值范围。",
        ...
        fieldDesc: "场景名称为下拉选择框，默认值为「请选择」；意向级别为多选下拉框，支持勾选多个已启用的级别，默认显示「请选择」。",
        interactionDesc: "未选择场景名称时，隐藏最后通话用户意向级别筛选项；选择场景名称后，显示级别筛选项，并按当前场景对应的供应商标签结果刷新级别选项；支持点击整行勾选/取消勾选，框内以逗号隔开显示已选项，点击页面外部任意区域可自动收起下拉面板；重置筛选时清空场景名称与级别勾选并重新隐藏级别筛选项。",
        judgeRule: "必须选中具体场景名称后才能使用级别筛选；若多选了多个意向级别，则系统应用「或（OR）逻辑」同时匹配符合这几项意向级别的线索记录。",
        exceptionRule: "若选中场景未配置场景类型、未关联供应商标签，或当前租户类型下没有可用供应商标签，则最后通话用户意向级别显示「暂无级别」且无可选项。",
        otherDesc: "该逻辑消费标签管理页供应商标签集的配置 and 映射结果。"
      },
      desc: "1. 功能名称：场景名称与多选级别筛选<br>2. 功能说明：用于在线索记录中选择具体外呼场景，并驱动「最后通话用户意向级别（多选）」筛选项。<br>3. 权限范围：当前登录账号可查看的租户范围。<br>4. 数据来源：业务场景列表、登录账号租户信息、标签管理中的供应商标签集配置。<br>5. 取值逻辑：场景名称先按登录账号租户类型过滤；选中具体场景后，读取该场景关联的场景类型，再按「登录账号租户类型 + 场景类型」查询对应关联的供应商标签，最后通话用户意向级别取这些供应商标签映射后的意向标签集合。<br>6. 字段说明：场景选择为单选下拉框；级别选择为多选下拉框（带 Checkbox），默认「请选择」。<br>7. 交互说明：未选场景时隐藏级别筛选；选中场景后显示并刷新多选选项，支持点击行勾选，已选项以逗号隔开，点击页面空白处自动收起；重置后清空并隐藏。<br>8. 判断规则：多选意向级别时，按「或（OR）」匹配查询过滤。<br>9. 异常规则：无关联供应商标签时，级别选项为空或显示「暂无级别」。<br>10. 其他说明：该逻辑消费标签管理页供应商标签集的配置和映射结果。"
    }
  ]
};
