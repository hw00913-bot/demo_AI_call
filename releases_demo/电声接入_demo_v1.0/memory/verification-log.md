...
## 2026-08-20 黑名单号码详情字段验证

- JavaScript 语法、HTML 解析和差异格式检查通过。
- 实际打开“外呼黑名单—黑名单号码详情”，详情仅展示用户号码、所属分组、添加类型、添加原因、添加人、添加时间和有效期。
- 弹窗不含“客户名称”和“来源”，字段网格自动补位。

## 2026-08-19 手工导入一号码一批次验证

- JavaScript 语法、HTML 解析和差异格式检查通过。
- 手工导入弹窗仅保留上传区，不含新建批次、继续批次或完成状态配置。
- 导入记录表头为“生成批次数”，两条示例均为“已完成”。
- 模拟导入 3 个号码后生成 3 个独立导入/执行批次，每个批次线索数为 1。

## 2026-08-19 电声拦截状态映射验证

- 电声 `leadStatus=BLOCKED` / `finalCallResult=BLOCKED` 模拟数据含本地状态“黑名单过滤”。
- 已过滤名单展示“黑名单过滤”，不再展示“过滤/拉黑（BLOCKED）”。
- 用户主动终止的本地记录仍展示“终止外呼 (TERMINATED)”，不会被默认映射为黑名单过滤。
...
## 2026-07-23 中台通话状态映射验证

- 通话记录和线索记录的状态筛选均加载 25 项中台完整枚举，包含新增的“秒挂、伪接通”，顺序与需求一致。
- 电声示例记录验证：`301` 展示“已接通”，`302` 展示“秒挂”，`303` 展示“伪接通”，`205` 展示“拒接”，`206` 映射“无人接听”。
- 主状态兜底验证：`answerMainStatus=3` 映射“已接通”，`answerMainStatus=2` 映射“无法接通”。
- 页面无横向溢出，浏览器控制台无错误；JavaScript、HTML 和差异格式检查通过。
- `BLOCKED` 继续只用于呼叫名单“已过滤”判断，具体拦截类通话状态必须结合来源。
