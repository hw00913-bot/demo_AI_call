...
S4 在生成项目内是结构确认阶段，不负责执行创建脚本。创建、迁移、底座裁剪和工具回灌由 loop 源仓库的 `skill-library/create-prototype-project/scripts/create_project.py` 完成；生成项目 runtime package 只保留 S4 规则和调度上下文。
...
- 如果 PM 手动生成并回写 `annotations.js`，交付门禁会继续校验 `sourceRefs` 和 `fieldRefs`。
