...
- 目标目录已有 `config/workflow.json`：**在读取项目内任何工具前**，先从当前总控技能包调用 `skill-library/create-prototype-project/scripts/create_project.py "目标目录" --sync-runtime`。该动作幂等刷新项目内确定性脚本、阶段技能和交付分页壳，不覆盖业务文件、memory、annotations、`docs/interaction.html` 正文或已有 ProcessOn 链接。随后再读取 `python3 tools/loop_run.py status .` 和 `python3 tools/loop_run.py dispatch .`；不能因为缓存存在跳过 S0-S3。
