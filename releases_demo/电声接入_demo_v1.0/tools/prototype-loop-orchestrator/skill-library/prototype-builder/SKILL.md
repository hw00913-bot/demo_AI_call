...
4. 每个新增或关键可解释区域优先补稳定 `data-anno` 锚点；锚点值在本轮全部业务源码中必须全局唯一，但不写标注数据。
5. 保留并验证统一交付分页：`index.html`、`docs/index.html`、`flowcharts/index.html` 都必须加载 `js/delivery-nav.js`，让 PM 能直接切换“原型页面 / 说明文档 / 流程图集”；业务导航重构不能移除该入口。
