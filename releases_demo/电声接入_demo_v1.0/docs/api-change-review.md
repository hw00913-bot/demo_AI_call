...
| 导入完成 | 导入接口新增必填 `isCompleted`，完成后由策略自动进入执行流程 | 呼叫名单文档需按“导入批次完成”描述，不再依赖单独启动接口 |
...
当前项目统一以 `strategyCode` 作为任务引用键。线索导入传入 `strategyCode`，并在 `isCompleted=true` 时进入执行流程；20260713 版已删除独立启动接口。
