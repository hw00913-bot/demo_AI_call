  ...
  "result-records": [
    {
      ...
      "sections": {
        ...
        "valueLogic": "通话状态由 answerMainStatus/answerStatus 映射为已接听、拒接、无应答、忙线、停机等前台状态；通话时长取 callDurationSeconds。",
        "fieldDesc": "用户号码、通话开始时间、通话结束时间、通话时长、场景名称、通话状态、外呼总结、智能平台、最后通话节点、操作。",
        "interactionDesc": "表格列表，支持按用户号码、场景名称、通话状态、智能平台筛选。点击详情打开通话详情弹窗。",
        "judgeRule": "未接通或通话异常的记录可能无小结。",
        "exceptionRule": "无数据时展示空状态。",
        "otherDesc": "当前原型映射：MockCallRecordRows。加微状态 wechatStatus 在详情抽屉中展示。"
      },
      "desc": "1. 功能名称：电声通话记录列表\n2. 功能说明：查看每通电声外呼的时间、时长、状态、小结。加微状态在详情抽屉中核对。\n3. 权限范围：超管/租户管理员/租户运营可见。\n4. 数据来源：电声 D02 话单回调。\n5. 取值逻辑：answerMainStatus/answerStatus 映射为前台状态。\n6. 字段说明：号码、时间、时长、场景、状态、小结、平台、最后节点、操作。\n7. 交互说明：表格列表 → 筛选 → 点击详情打开弹窗。\n8. 判断规则：未接通可能无小结。\n9. 异常规则：无数据展示空状态。\n10. 其他说明：当前原型映射：MockCallRecordRows。",
