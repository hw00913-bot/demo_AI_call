...
window.Pages['sys-tags'] = (function() {
  ...
  function buildSceneModalHTML() {
    ...
    html += '<div class="modal-content" data-anno="sys-tags-scene-modal" style="background:#fff;border-radius:8px;width:800px;max-width:calc(100vw - 40px);max-height:70vh;display:flex;flex-direction:column;box-shadow:0 4px 24px rgba(0,0,0,0.15);">';
    ...
    html += '</div>';
    html += '<div class="modal-footer scene-config-footer">';
    html += '<span class="scene-sync-note">场景在中台标签集和供应商标签集中统一生效。</span>';
    html += '<button class="btn-add-scene">+ 新增场景</button>';
    html += '</div>';
    html += '</div></div>';
    return html;
  }
  ...
  function buildSceneTableHTML() {
    ...
    html += '<thead><tr><th style="width:60px;">序号</th><th style="width:150px;">本地编码</th><th>场景名称</th><th style="width:75px;">状态</th><th style="width:220px;">操作</th></tr></thead>';
    html += '<tbody>';
    MockTagScenes.forEach(function(sc, idx) {
      ...
      html += '<input type="text" class="scene-name-input" value="' + escapeHTML(sc.name) + '" style="display:none;width:100%;padding:4px 8px;border:1px solid #d9d9d9;border-radius:4px;font-size:13px;">';
      html += '</td>';
      html += '<td><span style="color:' + statusColor + ';">● ' + statusText + '</span></td>';
      html += '<td>';
      html += '<button class="btn-scene-edit" data-scene-id="' + sc.id + '">编辑</button>';
      html += '<button class="btn-scene-save" data-scene-id="' + sc.id + '" style="display:none;">保存</button>';
      html += '<button class="btn-scene-cancel" data-scene-id="' + sc.id + '" style="display:none;">取消</button>';
      html += '<button class="btn-scene-toggle" data-scene-id="' + sc.id + '">' + (sc.status === 'enabled' ? '停用' : '启用') + '</button>';
      html += '<button class="btn-scene-delete" data-scene-id="' + sc.id + '">删除</button>';
      html += '</td>';
      html += '</tr>';
    });
    html += '</tbody></table>';
  ...
  function buildStylesHTML() {
    return '<style>' +
      ...
      '.scene-config-footer { display:flex;align-items:center;justify-content:space-between;padding:12px 20px;border-top:1px solid #f0f0f0; }' +
      '.scene-sync-note { color:#8c8c8c;font-size:12px;line-height:20px; }' +
      '.btn-add-scene { padding:6px 16px;background:#1677ff;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:13px; }' +
      '.btn-scene-edit, .btn-scene-save, .btn-scene-cancel, .btn-scene-toggle, .btn-scene-delete { margin-right:4px;padding:3px 8px;font-size:12px;border-radius:3px;cursor:pointer;background:#fff; }' +
      '.btn-scene-edit, .btn-scene-cancel { border:1px solid #d9d9d9;color:#333; }' +
      '.btn-scene-save { border:1px solid #52c41a;color:#389e0d; }' +
      '.btn-scene-toggle { border:1px solid #faad14;color:#d48806; }' +
      '.btn-scene-delete { border:1px solid #ff4d4f;color:#ff4d4f; }' +
  ...
  function autoSelectFirst() {
    var firstSupplier = MockTagSuppliers[0];
    var firstTT = MockTenantTypes[0];
    var firstSc = MockTagScenes.find(function(scene) { return scene.status === 'enabled'; });
    if (!firstSupplier || !firstTT || !firstSc) return;

    var supplierRoot = document.querySelector('.tree-supplier-root');
    if (supplierRoot) toggleTreeNode(supplierRoot);

    var supplierNode = document.querySelector('.tree-supplier[data-supplier="' + firstSupplier.id + '"]');
    if (supplierNode) toggleTreeNode(supplierNode);

    var tenantNode = document.querySelector('.tree-tenant[data-supplier="' + firstSupplier.id + '"][data-tenant="' + firstTT.id + '"]');
    if (tenantNode) toggleTreeNode(tenantNode);

    var node = document.querySelector('.tree-scene[data-supplier="' + firstSupplier.id + '"][data-tenant="' + firstTT.id + '"][data-scene="' + firstSc.id + '"]');
    if (node) selectSceneNode(node);
  }
