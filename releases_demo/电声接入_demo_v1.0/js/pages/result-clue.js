...
(function () {
  ...
  var allRows = [
    { phone: '138****0001', leadId: 'LEAD_DS_001', platform: '电声平台', attemptCount: 1, lastVisitTime: '2026-07-02 10:15:00', visitCount: 1, callStatus: '已接通', record: '客户确认周日到店并关注首保礼包。', level: 'A(高意向)', tags: ['高意向', '周日到店', '已加微'], sceneName: '东风日产-新线索-内部调优', firstVisitTime: '2026-07-02 10:15:00', secondVisitTime: '-', thirdVisitTime: '-', detailTags: { intent: 'A(高意向)', concern: '首保礼包', city: '广州', arrivalTime: '周日', buyTime: '近期', callTag: '已加微' }, summary: '客户确认周日到店并关注首保礼包，建议门店及时承接。' },
    { phone: '139****0002', leadId: 'LEAD_DS_002', platform: '电声平台', attemptCount: 1, lastVisitTime: '2026-07-02 09:30:10', visitCount: 1, callStatus: '秒挂', record: '客户接通后表示忙碌并挂断。', level: 'D(无意向)', tags: ['无意向', '稍后联系'], sceneName: '东风日产-冷线索-上线验证', firstVisitTime: '2026-07-02 09:30:10', secondVisitTime: '-', thirdVisitTime: '-', detailTags: { intent: 'D(无意向)', concern: '客户忙碌', city: '上海', arrivalTime: '-', buyTime: '-', callTag: '稍后联系' }, summary: '客户接通后表示忙碌并挂断，可按任务策略继续触达。' },
    { phone: '181****0003', leadId: 'LEAD_DS_003', platform: '电声平台', attemptCount: 2, lastVisitTime: '2026-07-02 09:12:00', visitCount: 2, callStatus: '拒接', record: '-', level: '-', tags: ['需重拨'], sceneName: '东风日产-新线索-内部调优', firstVisitTime: '2026-07-02 08:45:00', secondVisitTime: '2026-07-02 09:12:00', thirdVisitTime: '-', detailTags: { intent: '-', concern: '-', city: '深圳', arrivalTime: '-', buyTime: '-', callTag: '需重拨' }, summary: '本次拒接，执行批次已记录 2 次拨打。' },
    { phone: '185****0004', leadId: 'LEAD_DS_004', platform: '电声平台', attemptCount: 1, lastVisitTime: '2026-07-01 16:22:00', visitCount: 1, callStatus: '已接通', record: '客户表示车辆已售，无需继续跟进。', level: 'D(无意向)', tags: ['无意向', '车辆已售'], sceneName: '东风日产-冷线索-上线验证', firstVisitTime: '2026-07-01 16:22:00', secondVisitTime: '-', thirdVisitTime: '-', detailTags: { intent: 'D(无意向)', concern: '车辆已售', city: '上海', arrivalTime: '-', buyTime: '已购车', callTag: '无需回访' }, summary: '客户车辆已售，无需继续跟进。' },
  ...
  function showDetail(index) {
    ...
    document.body.insertAdjacentHTML('beforeend', '<div class="clue-visit-backdrop" id="clueVisitBackdrop" onclick="window.Pages[\'result-clue\'].closeVisitModal(event)">' +
      ...
      '<th>序号</th><th>回访轮次</th><th>用户号码</th><th>回访记录</th><th>客户详细标签</th><th>意向级别</th><th>通话状态</th><th>通话时长</th><th>是否重拨</th><th>通话开始时间</th><th>通话结束时间</th><th>操作</th>' +
      '</tr></thead><tbody><tr>' +
      ...
      '<td>' + escapeHTML(item.level || '-') + '</td><td>' + escapeHTML(item.callStatus || '-') + '</td><td>16秒</td><td>' + (typeof item.attemptCount === 'number' ? (item.attemptCount > 1 ? '是' : '否') : '-') + '</td>' +
  ...
  function showCallDetailByPhone(phone) {
    ...
    var callIndex = Array.isArray(window.MockCallRecordRows) ? window.MockCallRecordRows.findIndex(function (record) { return record.phone === phone; }) : -1;
    if (callIndex >= 0 && window.Pages['result-records'] && typeof window.Pages['result-records'].showDetail === 'function') {
      window.Pages['result-records'].showDetail(callIndex);
      return;
    }
  ...
  function render() {
    return '<div class="result-clue-page">' +
      ...
      '<div class="clue-filter-item"><label>最后通话状态：</label><select id="clueStatus" class="clue-select"><option value="">请选择</option>' + window.renderMiddleCallStatusOptions() + '</select></div>' +
      ...
      '</div>' +
      '<div class="clue-table-panel" data-anno="result-clue-list">' +
